...
int ucp_phy_read(int regnum)
{
	struct phy_device *phydev = &g_phydev;
	
	return eqos_mdio_read(NULL, phydev->addr, 0, regnum);
}
int ucp_phy_write(int regnum, u16 val)
{
	struct phy_device *phydev = &g_phydev;

	return eqos_mdio_write(NULL, phydev->addr, 0, regnum, val);
}
...
int ucp_phy_reset()
{
	...
	ucp_phy_write(MII_BMCR, BMCR_RESET);
...
int ucp_genphy_update_link(void)
{
...
static int genphy_setup_forced(void)
{
	...
	err = ucp_phy_write(MII_BMCR, ctl);
...
int genphy_config_advert(void)
{
	...
	adv = ucp_phy_read(MII_ADVERTISE);
	oldadv = adv;
	...
	if (adv != oldadv) {
		err = ucp_phy_write(MII_ADVERTISE, adv);
	...
	err = ucp_phy_write(MII_CTRL1000, adv);
	if (err < 0)
		return err;

	return changed;
}
int ucp_genphy_restart_aneg(void)
{
	int ctl;

	ctl = ucp_phy_read(MII_BMCR);
	...
	ctl = ucp_phy_write(MII_BMCR, ctl);

	return ctl;
}


int ucp_genphy_config_aneg(void)
{
	int result;
	/*前期调试，强制设置，取消自动协商功能*/
	 ...
	 */
	if (result > 0)
		result = ucp_genphy_restart_aneg();
...
int m88e1xxx_parse_status(void)
{
	...
	mii_reg = ucp_phy_read(MIIM_88E1xxx_PHY_STATUS);
...
int m88e1011s_startup(void)
{
	...
	ret = ucp_genphy_update_link();
	if (ret)
		return ret;

	return m88e1xxx_parse_status();
}
void my_phy_init(void)
{
	struct phy_device *phydev = &g_phydev;
	phydev->speed = SPEED_100;
	phydev->duplex = DUPLEX_FULL;
...
int m88e1518_config(void)
{
	...
	/* EEE initialization */
	ucp_phy_write(MIIM_88E1118_PHY_PAGE, 0x00ff);
	ucp_phy_write( 17, 0x214B);
	ucp_phy_write(16, 0x2144);
	ucp_phy_write(17, 0x0C28);
	ucp_phy_write(16, 0x2146);
	ucp_phy_write(17, 0xB233);
	ucp_phy_write(16, 0x214D);
	ucp_phy_write(17, 0xCC0C);
	ucp_phy_write(16, 0x2159);
	ucp_phy_write(MIIM_88E1118_PHY_PAGE, 0x0000);

	/* led initialization */
//	ucp_phy_write(MIIM_88E1118_PHY_LED_PAGE, 0x00ff);
//	ucp_phy_write( 16, 0x0);
//	ucp_phy_write(MIIM_88E1118_PHY_LED_PAGE, 0x0000);


	/* rgmii init */
	ucp_phy_write(MII_MARVELL_PHY_PAGE, 2);

	reg = ucp_phy_read(MIIM_88E151x_PHY_MSCR);
	reg &= ~MIIM_88E151x_RGMII_RXTX_DELAY;

	reg |= MIIM_88E151x_RGMII_RXTX_DELAY;

	ucp_phy_write(MIIM_88E151x_PHY_MSCR, reg);

	ucp_phy_write(MII_MARVELL_PHY_PAGE, 0);


	/* soft reset */
	ucp_phy_reset();

	ucp_genphy_config_aneg();
//	ucp_genphy_restart_aneg();/* it will make phy-mode 10M  to 100M */
...
u32 ucp_get_phy_id(void)
{
	...
	reg = ucp_phy_read(MII_PHYSID1);
	id = reg;
	reg = ucp_phy_read(MII_PHYSID2);
	id = id<<16;
	id |= reg;
	phydev->phy_id = id;
