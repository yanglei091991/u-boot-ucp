...
int wait_for_bit_le32(u32 *reg, u32 mask, bool set)
{
	u32 val;							
...
int clrsetbits_le32(u32*reg, u32 clr, u32 set)
{
	*reg &= ~ clr;
	*reg |= set;
	return 0;
...
void setbits_le32(u32 *reg, u32 mask)
{
	*(volatile u32 *)reg |= mask;
}
void clrbits_le32(u32 *reg, u32 mask)
{
	u32 val = 0;
	*(volatile u32 *)reg &= ~mask;
...
static int eqos_mdio_wait_idle(void)
{
	...
	return wait_for_bit_le32(&eqos->mac_regs->mdio_address,EQOS_MAC_MDIO_ADDRESS_GB, false);
}
...
static int eqos_adjust_link(void)
{
	...
	if (phydev->duplex)
		ret = eqos_set_full_duplex();
	else
		ret = eqos_set_half_duplex();
	if (ret < 0) {
		return ret;
	}
	...
	case SPEED_100:
		en_calibration = true;
		ret = eqos_set_mii_speed_100();
		break;
	case SPEED_10:
		en_calibration = true;
		ret = eqos_set_mii_speed_10();
		break;
	default:
		return -EINVAL;
	}
